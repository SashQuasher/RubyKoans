...
class AboutMessagePassing < Neo::Koan
  ...
  def test_sending_a_message_with_arguments
    ...
    assert_equal [3,4,nil,6], mc.send(:add_a_payload, 3, 4, nil, 6) #Need to indicadte the payload method
  end
  ...
  def test_calling_method_missing_causes_the_no_method_error
    ...
    exception = assert_raise(Exception) do
      typical.method_missing(:foobar) #till Not possible
    end
    assert_match(/foobar/, exception.message)
  ...
  def test_non_foo_messages_are_treated_normally
    ...
    assert_raise(Exception) do
      catcher.normal_undefined_method #There is no response programmed for this method call
    end
  end
