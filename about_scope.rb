...
class AboutScope < Neo::Koan
  ...
  def test_dog_is_not_available_in_the_current_scope
    assert_raise(Exception) do
      Dog.new #Dog Class is Inaccesible as it is inside the modules.
    end
  end
  ...
  def test_you_can_reference_nested_classes_using_the_scope_operator
    ...
    assert_equal :joes_dog, rover.identify #Each one has it's own default identty symbol
  ...
  def test_bare_bones_class_names_assume_the_current_scope
    assert_equal true, AboutScope::String == String #It doesn't have the same methods nor properties
  end
  ...
  def test_use_the_prefix_scope_operator_to_force_the_global_scope
    assert_equal true, ::String == "HI".class #Specifies the class out of this scope
  end
  ...
  def test_class_names_are_just_constants
    assert_equal true, MyString == ::String #Classes name have constant Values
    assert_equal true, MyString == "HI".class
  end

  def test_constants_can_be_looked_up_explicitly
    assert_equal true, PI == AboutScope.const_get("PI") #Search the class in a specified scope
